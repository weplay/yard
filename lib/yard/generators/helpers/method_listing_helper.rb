module YARD
  module Generators::Helpers
    module MethodListingHelper
      def has_methods?(object)
        method_list.size > 0
      end
      
      def has_inherited_methods?
      ...
      def inherited_meths_by_class
        all_meths = object.inherited_meths(:scope => scope, :visibility => visibility)
        object.inheritance_tree[1..-1].each do |superclass|
      ...
      def included_meths_by_module
        all_meths = object.included_meths(:scope => scope, :visibility => visibility)
        object.mixins.each do |mixin|
